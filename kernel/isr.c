...
void fault_handler(uint32_t int_no)
{
...
void irq_handler(uint32_t irq_no)
{
    ...
    if (irq_no >= 8)
    {
        __outb(0xA0, 0x20); // Send EOI to slave PIC
    }
    __outb(0x20, 0x20); // Send EOI to master PIC

    // Call specific IRQ handlers
    switch (irq_no)
    {
        case 0:
            pit_handler();
...
void irq_install()
{
    // Install IRQ handlers (IRQ 0-15 mapped to ISR 32-47)
